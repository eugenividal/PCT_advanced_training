...
plot(z$geometry)
plot(l_msoa_highest_cycling$geometry, col = "green",  lwd=4.0, add = TRUE)
```

```{r}
# Top 5 desire lines driving
l_msoa = lines_all %>% 
  select(geo_code1, geo_code2, all, foot, bicycle, car_driver, rf_avslope_perc, rf_dist_km)

l_msoa_highest_car_driver <- top_n(l_msoa, n = 5, wt = car_driver)

plot(z$geometry)
plot(l_msoa_highest_car_driver$geometry, col = "red",  lwd=4.0, add = TRUE)
```

```{r}
# Get the LSOA files
zones_all_lsoa = get_pct_zones(region_name, geography = "lsoa")
lines_all_lsoa = get_pct_lines(region_name, geography = "lsoa")

z_lsoa = zones_all_lsoa %>% 
  select(geo_code, geo_name, all, foot, bicycle, car_driver)
```

```{r}
# Top 10 desire lines cycling
l_lsoa = lines_all_lsoa %>% 
  select(geo_code1, geo_code2, all, foot, bicycle, car_driver, rf_avslope_perc, rf_dist_km)

l_lsoa_highest_cycling <- top_n(l_lsoa, n = 10, wt = bicycle)

plot(z_lsoa$geometry)
plot(l_lsoa_highest_cycling$geometry, col = "green",  lwd=4.0, add = TRUE)
```
...
l_lsoa_highest_car_driver <- top_n(l_lsoa, n = 10, wt = car_driver)

plot(z_lsoa$geometry)
plot(l_lsoa_highest_car_driver$geometry, col = "red",  lwd=4.0, add = TRUE)
```
...
# Government Target scenario
l_msoa$pcycle = l_msoa$bicycle / l_msoa$all * 100
plot(l_msoa["pcycle_govtarget"], lwd = l_msoa$all / mean(l_msoa$all), breaks = c(0, 10, 20, 30, 40, 50))
```

```{r}
# Go dutch scenario
l_msoa$pcycle_go_dutch = uptake_pct_godutch(
  distance = l_msoa$rf_dist_km,
  gradient = l_msoa$rf_avslope_perc
  ) * 100 + l_msoa$pcycle
```
...
```{r}
l_msoa$pcycle = l_msoa$bicycle / l_msoa$all * 100
plot(l_msoa["pcycle_go_dutch"], lwd = l_msoa$all / mean(l_msoa$all), breaks = c(0, 10, 20, 30, 40, 50))
```
